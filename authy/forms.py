...
def ForbiddenUsers(value):
    forbidden_users = ['admin', 'css', 'js', 'authenticate', 'login', 'logout', 'root', 'administrator'
    'email', 'join', 'sql', 'insert','db', 'static', 'python', 'delete', 'TABLE', 'insert']
    if value.lower() in forbidden_users:
        raise ValidationError('Invalid name for users, this is a reserved word.')
...
class ChangePasswordForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not user.check_password(old_password):
            self._errors['old_password'] = self.error_class(['Old password does not match. Try again'])
...
class EditProfileForm(forms.ModelForm):
    ...
    url = forms.CharField(widget=forms.TextInput(), max_length=50, required=False)
    profile_info = forms.CharField(widget=forms.TextInput(), max_length=50, required=False)
